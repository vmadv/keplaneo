...
import { hoyISO, mesActualSlug } from "@/lib/dates";
...
// Pensado para dispararse 1 vez al mes (ver vercel.json). Regenera solo el
// mes en curso; si quieres adelantar el mes siguiente unos días antes de
// que acabe el actual, duplica esta ruta con `mesSiguienteSlug()`.
export async function GET(request: NextRequest) {
  ...
  for (const municipio of municipios as unknown as MunicipioConComunidadSlug[]) {
    try {
      ...
      // Sustituye el lote del mes en curso para este municipio, sin acumular
  ...
  return NextResponse.json({ fecha: hoy, mes, resultados });
}
